...
fnames = listdir('./new_fits/after division/known')
del fnames[0]


# make a function that gives a list of EW of a file
def ewlist(fname):
    data = pickle.load(open('./new_fits/after division/known/%s' % fname, 'rb'))
    ew_exp = [0] * len(data)
