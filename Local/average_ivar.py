...
def ivar_est(filename):
    ...
    # try:
        data_ivar = pickle.load(open('/Users/sarelg/gastro/sdss/no_smooth/none_trial/ivar/%s' % filename, 'rb'))
        data_flux = pickle.load(open('/Users/sarelg/gastro/sdss/no_smooth/none_trial/sub_exposures/%s' % filename, 'rb'))
        ...
        # flux_list = flux_list[~np.isnan(flux_list)]
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='/mnt/gastro/sarel/sdss/no_smooth/excels/average_ivar.log',
                        level=logging.INFO,
                        format='%(levelname)s: %(message)s')

    # create the file to write into
    path = '/mnt/gastro/sarel/sdss/no_smooth/'
    writer = csv.writer(open(path + 'excels/average_ivar.csv', 'w'))
    writer.writerow(['File name', 'avg_ivar'])
